...
const commands: Command[] = [
  // system commands
  {
    ...
    handler: () => ({
      message: [
        ...
        ...commands.map(
          (c) =>
            `    ${c.name}${(c.usage ? " " + c.usage : "").padEnd(18)} ${c.desc}`,
        ),
      ].join("\n"),
    }),
  },
    ...
    handler: async (args) => {
      ...
      if (sub === "trash") {
        try {
          ...
          return {
            message: "Semua dokumen rejected berhasil dihapus dari trash!",
          };
        } catch {
  ...
  },
  {
    ...
    handler: async (args) => {
      ...
      if (!name)
        return { message: "Usage: /ban [nama staff]\n  Contoh: /ban bambang" };
      ...
      try {
        ...
        const match = users.find(
          (u) =>
            u.name.toLowerCase() === name.toLowerCase() && u.role === "staff",
        );
        ...
        return {
          ...
          effect: () => {
            ...
            el.style.cssText =
              "position:fixed;inset:0;z-index:9999;pointer-events:none;display:flex;align-items:center;justify-content:center";
            el.innerHTML =
              "<div style='background:#ff4655;color:white;padding:24px 48px;font-size:24px;font-weight:900;letter-spacing:4px;clip-path:polygon(0 0,calc(100% - 12px) 0,100% 12px,100% 100%,12px 100%,0 calc(100% - 12px));animation:secret-ban-fade 2s ease forwards'>BANNED: " +
              match.name.toUpperCase() +
              "</div>";
            ...
            style.textContent =
              "@keyframes secret-ban-fade{0%{opacity:0;transform:scale(0.8)}30%{opacity:1;transform:scale(1.05)}60%{opacity:1;transform:scale(1)}100%{opacity:0;transform:scale(0.9)}}";
            document.head.append(style);
            setTimeout(() => {
              el.remove();
              document.getElementById("secret-ban-style")?.remove();
            }, 2500);
          },
        };
      } catch (err: unknown) {
  ...
  },
  {
    ...
    handler: async () => {
      try {
        ...
        return {
          message: "Dokumen rejected di trash: " + (data.rejected ?? 0),
        };
      } catch {
